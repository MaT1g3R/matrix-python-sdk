...
class Room(object):
    ...
    async def set_user_profile(self,
                               displayname=None,
                               avatar_url=None,
                               reason="Changing room profile information"):
        member = await self.client.api.get_membership(self.room_id,
                                                      self.client.user_id)
    ...
    async def get_display_name(self):
        ...
        members = await self.get_joined_members()
        # members without me
        members[:] = [await u.get_display_name() for u in members if
    ...
    async def send_text(self, text):
        ...
        """
        return await self.client.api.send_message(self.room_id, text)
    ...
    async def send_html(self, html, body=None, msgtype="m.text"):
        ...
        """
        return await self.client.api.send_message_event(
            self.room_id, "m.room.message",
            self.get_html_content(html, body, msgtype))

    async def set_account_data(self, type, account_data):
        return await self.client.api.set_room_account_data(
            self.client.user_id, self.room_id, type, account_data)

    async def get_tags(self):
        return await self.client.api.get_user_tags(self.client.user_id,
                                                   self.room_id)

    async def remove_tag(self, tag):
        return await self.client.api.remove_user_tag(
            self.client.user_id, self.room_id, tag
        )

    async def add_tag(self, tag, order=None, content=None):
        return await self.client.api.add_user_tag(
            self.client.user_id, self.room_id,
            tag, order, content
        )

    async def send_emote(self, text):
        """ Send a emote (/me style) message to the room.

        Args:
            text (str): The message to send
        """
        return await self.client.api.send_emote(self.room_id, text)

    async def send_file(self, url, name, **fileinfo):
        ...
        return await self.client.api.send_content(
            self.room_id, url, name, "m.file",
            extra_information=fileinfo
        )

    async def send_notice(self, text):
        return await self.client.api.send_notice(self.room_id, text)

    # See http://matrix.org/docs/spec/r0.0.1/client_server.html#m-image for the
    # imageinfo args.
    async def send_image(self, url, name, **imageinfo):
    ...
    async def send_location(self, geo_uri, name, thumb_url=None, **thumb_info):
        ...
        """
        return await self.client.api.send_location(self.room_id, geo_uri, name,
                                                   thumb_url, thumb_info)

    # See http://matrix.org/docs/spec/client_server/r0.2.0.html#m-video for the
    # videoinfo args.
    async def send_video(self, url, name, **videoinfo):
        ...
        """
        return await self.client.api.send_content(self.room_id, url, name,
                                                  "m.video",
                                                  extra_information=videoinfo)

    # See http://matrix.org/docs/spec/client_server/r0.2.0.html#m-audio for the
    # audioinfo args.
    async def send_audio(self, url, name, **audioinfo):
    ...
    async def redact_message(self, event_id, reason=None):
    ...
    async def kick_user(self, user_id, reason=""):
        ...
        """
        try:
            await self.client.api.kick_user(self.room_id, user_id, reason)
            return True
        except MatrixRequestError:
            return False
    ...
    async def ban_user(self, user_id, reason):
        ...
        """
        try:
            await self.client.api.ban_user(self.room_id, user_id, reason)
            return True
        except MatrixRequestError:
            return False
    ...
    async def unban_user(self, user_id):
        ...
        """
        try:
            await self.client.api.unban_user(self.room_id, user_id)
            return True
        except MatrixRequestError:
            return False
    ...
    async def leave(self):
        ...
        """
        try:
            await self.client.api.leave_room(self.room_id)
    ...
    async def update_room_name(self):
        ...
        """
        try:
            response = await self.client.api.get_room_name(self.room_id)
    ...
    async def set_room_name(self, name):
        """ Set room name
        ...
        """
        try:
            await self.client.api.set_room_name(self.room_id, name)
            self.name = name
            return True
        except MatrixRequestError:
            return False

    async def send_state_event(self, event_type, content, state_key):
        ...
        """
        return await self.client.api.send_state_event(
            self.room_id,
            event_type,
            content,
            state_key
    ...
    async def update_room_topic(self):
        """ Get room topic
        ...
        """
        try:
            response = await self.client.api.get_room_topic(self.room_id)
    ...
    async def set_room_topic(self, topic):
        """ Set room topic
        ...
        """
        try:
            await self.client.api.set_room_topic(self.room_id, topic)
            self.topic = topic
            return True
        except MatrixRequestError:
            return False
    ...
    async def update_aliases(self):
        ...
        """
        try:
            response = await self.client.api.get_room_state(self.room_id)
    ...
    async def add_room_alias(self, room_alias):
        ...
        """
        try:
            await self.client.api.set_room_alias(self.room_id, room_alias)
            return True
        except MatrixRequestError:
            return False
    ...
    async def get_joined_members(self):
        ...
        response = await self.client.api.get_room_members(self.room_id)
    ...
    async def backfill_previous_messages(self, reverse=False, limit=10):
        ...
        """
        res = await self.client.api.get_room_messages(self.room_id,
                                                      self.prev_batch,
                                                      direction="b",
                                                      limit=limit)
        events = res["chunk"]
    ...
    async def modify_user_power_levels(self, users=None, users_default=None):
        ...
        """
        try:
            content = await self.client.api.get_power_levels(self.room_id)
            if users_default:
            ...
            await self.client.api.set_power_levels(self.room_id, content)
            return True
        except MatrixRequestError:
            return False

    async def modify_required_power_levels(self, events=None, **kwargs):
